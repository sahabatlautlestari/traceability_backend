...
class BuyersHandler {
  constructor(service, validator) {
    ...
    this.getBuyersByBuyerIdHandler = this.getBuyersByBuyerIdHandler.bind(this);
  ...
  async postBuyerHandler(request, h) {
    try {
      ...
      const { idbuyer, buyername, location } = request.payload;
      //const { id: credentialId } = request.auth.credentials;

      const buyerId = await this._service.addBuyer({
        idbuyer, buyername, location,
      });
  ...
  async getBuyersByBuyerIdHandler(request, h) {
    try {
      const { idbuyer = '' } = request.query;
      //const { id: credentialId } = request.auth.credentials;

      //await this._service.verifyBuyersAccess(id, credentialId);
      const buyers = await this._service.getBuyersByBuyerId(idbuyer);
